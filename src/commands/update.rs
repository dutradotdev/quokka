...
const INSTALLER_URL: &str =
    "https://github.com/dutradotdev/quokka/releases/latest/download/quokka-cli-installer.sh";
...
async fn run_installer() -> Result<()> {
    ...
    let script = format!(
        "set -o pipefail 2>/dev/null; curl --proto '=https' --tlsv1.2 -LsSf {INSTALLER_URL} | sh"
    );
